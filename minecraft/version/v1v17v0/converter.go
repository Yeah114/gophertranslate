...
// NewVersionConverter creates a version converter for Minecraft 1.17.0.
...
func (c *VersionConverter) StartGame(data *minecraft.GameData) (err error) {
	if c.c.ClientConnEcho().Proto().ID() < c.c.ServerConnEcho().Proto().ID() {
		...
		for _, state := range BlockStates {
			...
			if err != nil {
				return fmt.Errorf("v1v17v0.VersionConverter.StartGame: failed to register custom block: %w", err)
			}
		}
...
func (c *VersionConverter) HandlePacket(pk packet.Packet, sender define.Conn) (err error) {
	...
	return fmt.Errorf("v1v17v0.VersionConverter.HandlePacket: unknown sender")
}
